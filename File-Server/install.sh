...
################################################################################
# File Server Setup - Interactive Installation Script
# 
# This script provides an interactive interface to deploy file sharing and
# backup services on a Proxmox Virtual Environment host.
#
...
# Services for File Server: storage, sharing, and backup
declare -A SERVICES=(
...
declare -A SERVICE_DESCRIPTIONS=(
    ...
    [samba]="Share folders with Windows/macOS/Linux clients. Native SMB/CIFS protocol."
    [syncthing]="Sync files across multiple devices in real-time. Privacy-focused continuous sync."
...
print_header() {
    echo -e "${BLUE}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║            File Server Setup - Interactive Installer           ║"
...
main() {
    ...
    echo "  1. Mount storage volumes for Nextcloud and Samba"
    echo "  2. Configure Samba shares and permissions"
    echo "  3. Set up Syncthing device folders"
    echo "  4. Access Netdata monitoring dashboard"
    echo ""
